...
export class ExportKwhReportComponent implements OnInit {
  ...
  setDefaultReportDates(): void {
    const today = new Date();
    const lastMonth = new Date();
    lastMonth.setMonth(today.getMonth() - 1);
  
    // Ensure the day exists in last month (handle cases where the previous month has fewer days)
    if (today.getDate() > new Date(lastMonth.getFullYear(), lastMonth.getMonth() + 1, 0).getDate()) {
      lastMonth.setDate(new Date(lastMonth.getFullYear(), lastMonth.getMonth() + 1, 0).getDate());
    } else {
      lastMonth.setDate(today.getDate());
    }
  
    this.startDate = lastMonth.toISOString().split('T')[0]; // Last month same day
    this.endDate = today.toISOString().split('T')[0];       // Today (daily)
  
    const year = lastMonth.getFullYear();
    const month = (lastMonth.getMonth() + 1).toString().padStart(2, '0');
  
    this.startMonth = `${year}-${month}`;                   // Last month (monthly)
    this.endMonth = today.toISOString().slice(0, 7);        // Current month (monthly)
  }
  
  fetchAllTenants(): void {
    this.http.get<any[]>(`${this.apiBaseUrl}/get-all-tenants`).subscribe({
      next: (data) => {
        this.tenants = data.filter(tenant => tenant.isDeleted == false); // Only include active tenants
      },
      error: (err) => {
