...
public class BillingService {
    ...
    public Map<String, Double> getEnergyUsageForTenant(int tenantId, String month, String year) {
        // Ensure `tenantAndEnergyMeterService` is correctly fetching data
        return tenantAndEnergyMeterService.getEnergyMetersForTenant(tenantId, month, year);
    }

    public List<Map<String, Object>> getTenantsWithUsage(String month, String year) {
        List<Map<String, Object>> tenants = getAllTenants();
        List<Map<String, Object>> tenantUsageList = new ArrayList<>();

        for (Map<String, Object> tenant : tenants) {
            int tenantId = (Integer) tenant.get("id");

            // Fetch Energy Usage for this tenant
            Map<String, Double> usageData = getEnergyUsageForTenant(tenantId, month, year);
            double ebUsage = usageData.getOrDefault("totalEbKwh", 0.0);
            double dgUsage = usageData.getOrDefault("totalDgKwh", 0.0);

            tenant.put("eb_usage", ebUsage);
            tenant.put("dg_usage", dgUsage);

            tenantUsageList.add(tenant);
        }
    ...
    public List<Map<String, Object>> getAllTenants() {
        String sql = "SELECT id, name, unit_address FROM tenant";
